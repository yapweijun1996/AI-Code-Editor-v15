...
// --- Missing File Reading Functions ---
...
async function _readFileLines({ filename, start_line, end_line }, rootHandle) {
    if (!filename) throw new Error("The 'filename' parameter is required for read_file_lines.");
    if (!start_line) throw new Error("The 'start_line' parameter is required for read_file_lines.");
    if (!end_line) throw new Error("The 'end_line' parameter is required for read_file_lines.");
    ...
    try {
        ...
        const numberedLines = selectedLines.map((line, index) => `${start_line + index} | ${line}`);
        ...
        throw new Error(`Failed to read lines ${start_line}-${end_line} from file '${filename}': ${error.message}`);
    }
}
...
async function _readMultipleFiles({ filenames }, rootHandle) {
    if (!filenames || !Array.isArray(filenames)) {
        throw new Error("The 'filenames' parameter is required and must be an array for read_multiple_files.");
    }
    ...
    return {
        ...
        errors,
        total_files: filenames.length,
        successful_reads: results.length,
        failed_reads: errors.length
    };
}

// Search for pattern within a specific file
async function _searchInFile({ filename, pattern, context = 0 }, rootHandle) {
    if (!filename) throw new Error("The 'filename' parameter is required for search_in_file.");
    if (!pattern) throw new Error("The 'pattern' parameter is required for search_in_file.");
    ...
    try {
        ...
        const matches = [];
        const regex = new RegExp(pattern, 'gi'); // Case insensitive global search
        ...
        lines.forEach((line, index) => {
            if (regex.test(line)) {
                ...
                if (context > 0) {
                    const startContext = Math.max(0, index - context);
                    const endContext = Math.min(lines.length - 1, index + context);
                    
                    match.context_before = lines.slice(startContext, index).map((contextLine, contextIndex) => ({
                        line_number: startContext + contextIndex + 1,
                        content: contextLine
                    }));
                    
                    match.context_after = lines.slice(index + 1, endContext + 1).map((contextLine, contextIndex) => ({
                        line_number: index + contextIndex + 2,
                        content: contextLine
                    }));
                }
        ...
        throw new Error(`Failed to search in file '${filename}': ${error.message}`);
    }
}
